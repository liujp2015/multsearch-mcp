"""Interactive setup wizard for multsearch-mcp.
...
Usage:
    multsearch-setup                  # full interactive wizard
    multsearch-setup --print-only     # don't run claude, just print command

The questionary dependency is optional; install via:
    pip install 'multsearch-mcp[setup]'
    # or:  uvx --from 'multsearch-mcp[setup]' multsearch-setup
"""
...
try:
    ...
    print(
        ...
        "    pip install 'multsearch-mcp[setup]'\n"
        "或：uvx --from 'multsearch-mcp[setup]' multsearch-setup",
        file=sys.stderr,
    )
    sys.exit(1)
...
BANNER = r"""
╭──────────────────────────────────────────────────────────╮
│   multsearch-mcp · 引导式配置向导                          │
│   Tavily + Firecrawl + LLM  (+ Gemini grounding) ──► Claude │
╰──────────────────────────────────────────────────────────╯
"""
...
PROVIDER_PRESETS = {
    "ark": {
        "label": "火山方舟 Ark（推荐 · 已实测，glm-5.2 纯文本 + 国内直连）",
        "api_url": "https://ark.cn-beijing.volces.com/api/plan/v3",
        "model": "glm-5.2",
        "key_hint": (
            "注册 / 申请 Key：\n"
            "      · 控制台：https://console.volcengine.com/ark\n"
            "      · 创建 API Key（以 ark- 开头），开通 glm-5.2 模型\n"
            "    💡 Ark 走国内直连（无需代理）；模型为纯文本，由 Tavily/Firecrawl 找源后 summarize"
        ),
    },
    "custom": {
        "label": "自定义 OpenAI 兼容端点（手动填 base_url）",
        "api_url": "",
        "model": "glm-5.2",
        "key_hint": "任意 OpenAI 兼容 chat/completions 端点（内网网关 / 第三方代理站等）。",
    },
}
...
def step_provider() -> tuple[str, str, str, str]:
    ...
    if provider == "custom":
        api_url = _ask_text("自定义 base_url（OpenAI 兼容，如 https://your-gateway/v1）：")
    else:
...
def step_tavily() -> tuple[Optional[str], Optional[str]]:
    section("第 3 步 · Tavily（可选 · web_search 找源 + web_fetch / web_map）")
    info("未配置时，web_search 退化为只用 Firecrawl 一路；web_fetch / web_map 不可用。")
    info("注册：     https://app.tavily.com/home")
    info("💰 免费额度：1,000 次 API 调用 / 月（无需绑卡）。")
    if not _ask_confirm("现在配置 Tavily？", default=False):
        warn("已跳过 Tavily")
        return None, None
    key = _ask_text("TAVILY_API_KEY（控制台 → API Keys）：", secret=True,
                    validate=lambda v: True if v else "不能为空（如要跳过请选 No）")
    url = _ask_text("TAVILY_API_URL（回车使用默认）：", default="https://api.tavily.com")
    return key, url


def step_firecrawl() -> Optional[str]:
    section("第 4 步 · Firecrawl（可选 · web_search 找源 + Tavily 抓取失败时兜底）")
    info("注册：     https://www.firecrawl.dev/signin?view=signup")
    info("💰 免费额度：1,000 credits / 月（无需绑卡）。")
...
def step_gemini() -> tuple[Optional[str], Optional[str]]:
    section("第 5 步 · Gemini（可选 · 第二路独立搜索，Google 索引交叉验证）")
    info("未配置时，gemini_search 工具不可用；web_search 仍可用（Tavily + Firecrawl 一路）。")
    info("取 Key：   https://aistudio.google.com/apikey（免费）")
    info("⚠️ Google API 在中国大陆需走代理（HTTP_PROXY / HTTPS_PROXY）。")
    if not _ask_confirm("现在配置 Gemini？", default=False):
        warn("已跳过 Gemini — gemini_search 将不可用")
        return None, None
    key = _ask_text("GEMINI_API_KEY：", secret=True,
                    validate=lambda v: True if v else "不能为空（如要跳过请选 No）")
    model = _ask_text("GEMINI_MODEL（回车使用默认）：", default="gemini-2.5-flash")
    return key, model
...
def build_env(provider, api_url, api_key, model, tavily_key, tavily_url, firecrawl_key,
              gemini_key=None, gemini_model=None) -> dict[str, str]:
    env: dict[str, str] = {
        "MULT_PROVIDER": provider,
        "MULT_API_URL": api_url,
        "MULT_API_KEY": api_key,
        "MULT_MODEL": model,
    }
    if tavily_key:
...
def build_register_command(env: dict[str, str], package_spec: str, scope: str) -> list[str]:
    config_payload = {
        "type": "stdio",
        "command": "uvx",
        "args": ["--from", package_spec, "multsearch"],
        "env": env,
    }
    return [
        "claude", "mcp", "add-json", "multsearch",
        "--scope", scope,
...
def step_scope(default_scope: str) -> str:
    section("第 6 步 · 选择 MCP 作用域（决定这条配置写到哪里）")
...
def step_review_and_register(env: dict[str, str], package_spec: str, scope: str, print_only: bool) -> None:
    section("第 7 步 · 确认并注册到 Claude Code")
    ...
    try:
        subprocess.run(["claude", "mcp", "remove", "multsearch"], check=False,
    ...
    if result.returncode == 0:
        ok("已注册。重启 Claude Code 即可在 MCP 列表里看到 multsearch。")
    else:
        err("注册失败：")
        print(result.stdout)
        print(result.stderr)
...
def _resolve_provider_url(provider: str, custom_url: Optional[str]) -> tuple[str, str]:
    preset = PROVIDER_PRESETS.get(provider) or PROVIDER_PRESETS["ark"]
...
def run_non_interactive(args: argparse.Namespace) -> None:
    ...
    if not args.api_key:
        raise SystemExit("--api-key 是必填项（或设置环境变量 MULT_API_KEY）")
    ...
    print(f"\n==> 已注册到 scope={args.scope}。运行 `claude mcp list` 验证，重启 Claude Code 后 `/mcp` 即可看到 multsearch。")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="multsearch-mcp 配置向导（默认交互式；加 --non-interactive 走纯参数模式）",
    )
    parser.add_argument("--print-only", action="store_true", help="只打印命令，不执行 claude mcp add-json")
    parser.add_argument(
        "--package-spec",
        default=os.getenv("MULT_PACKAGE_SPEC", "multsearch-mcp"),
        help="uvx 安装时使用的包说明符（默认: multsearch-mcp；本地开发可填绝对路径）",
    )
    parser.add_argument(
        "--scope",
        choices=["project", "user", "local"],
        default=os.getenv("MULT_SCOPE", "project"),
    ...
    parser.add_argument("--provider", default="ark",
                        ...
                        help="provider（默认: ark）")
    parser.add_argument("--api-key", default=os.getenv("MULT_API_KEY"),
                        help="主 API Key（必填；Ark 以 ark- 开头）")
    parser.add_argument("--api-url", default=None,
                        help="自定义 base_url（仅 provider=custom 时需要）")
    parser.add_argument("--model", default=None,
                        help="模型名（默认走 provider 预设，ark=glm-5.2）")
    parser.add_argument("--tavily-key", default=os.getenv("TAVILY_API_KEY"),
                        help="Tavily Key（可选，不填则 web_fetch / web_map 不可用）")
    parser.add_argument("--firecrawl-key", default=os.getenv("FIRECRAWL_API_KEY"),
                        help="Firecrawl Key（可选，Tavily 失败时兜底 + web_search 找源）")
    parser.add_argument("--gemini-key", default=os.getenv("GEMINI_API_KEY"),
                        help="Gemini Key（可选，第二路独立搜索 gemini_search）")
    ...
    gemini_key, gemini_model = step_gemini()
    ...
    print("\n\033[1;32m完成。\033[0m 在 Claude Code 里运行 `/mcp` 应该能看到 multsearch。")
    if scope == "project":
